...
import { LogIn, User, Lock, ArrowLeft, Shield } from 'lucide-react';
...
function StaffLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.staffLogin(username, password);
      login(response.user, response.token);
      navigate('/staff/distribute');
    } catch (err) {
      ...
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8">
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-army-100 text-army-700 mb-4">
              <Shield className="w-8 h-8" />
            </div>
            <h1 className="text-2xl font-bold text-army-900 mb-2">工作人员登录</h1>
            <p className="text-gray-500">请使用工作人员账号登录</p>
          </div>
